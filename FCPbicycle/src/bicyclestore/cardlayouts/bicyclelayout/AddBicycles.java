...
public class AddBicycles extends JPanel {
	...
	private JTextField txtModel, txtColour, txtFrameSize, txtWheelSize, txtFrameComposition, txtCostPrice, txtSalePrice,
			txtNoOfGears;
	private JButton addBicycleButton;
	private JComboBox<String> productBox;
	private String[] productItems = { "Select a Product Type", "BMX", "Cruiser", "Hybrid", "Motorised Bike",
	...
	public void checkFieldsComplete(String name) {
		...
		// check if any text fields have been left blank and output error
		// message if true
		if (name.length() == 0 || model.length() == 0 || colour.length() == 0 || frameSize.length() == 0
				|| wheelSize.length() == 0 || noOfGears.length() == 0 || frameComposition.length() == 0
				|| costPrice.length() == 0 || salePrice.length() == 0) {
			JOptionPane.showMessageDialog(null, "Please Enter All Details", "Incomplete content",
					JOptionPane.ERROR_MESSAGE);
		}
		// attempt to add bicycle if all fields are complete
		else if (cbItemName.equals("BMX")) {

			database.addBicycle(new BMX(model, colour, intFrameSize, intWheelSize, frameComposition, doubleCostPrice,
					doubleSalePrice));
			clearTextFields();

		} else if (cbItemName.equals("Cruiser")) {

			database.addBicycle(new Cruiser(model, colour, intFrameSize, intWheelSize, frameComposition,
					doubleCostPrice, doubleSalePrice));
			clearTextFields();

		} else if (cbItemName.equals("Hybrid")) {

			database.addBicycle(new Hybrid(intNoOfGears, model, colour, intFrameSize, intWheelSize, frameComposition,
					doubleCostPrice, doubleSalePrice));
			clearTextFields();
		} else if (cbItemName.equals("Motorised Bike")) {

			database.addBicycle(new MotorisedBike(model, colour, intFrameSize, intWheelSize, frameComposition,
					doubleCostPrice, doubleSalePrice));
			clearTextFields();

		} else if (cbItemName.equals("Mountain Bike")) {

			database.addBicycle(new MountainBike(intNoOfGears, model, colour, intFrameSize, intWheelSize,
					frameComposition, doubleCostPrice, doubleSalePrice));
			clearTextFields();

		} else if (cbItemName.equals("Road Bike")) {

			database.addBicycle(new RoadBike(intNoOfGears, model, colour, intFrameSize, intWheelSize, frameComposition,
					doubleCostPrice, doubleSalePrice));
			clearTextFields();
	...
	private class ButtonActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			// if the user pressed the add bicycle button, check if fields are
			// all filled and attempt to add customer
