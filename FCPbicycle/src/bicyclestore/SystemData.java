...
public class SystemData {
	...
	private void createSupplierAccounts() {
		...
		catalog2.add(new RoadBike(18, "Boardman Road Comp", "Grey", 25, 28, "Alloy", 480, 649));
		catalog2.add(new RoadBike(11, "Cannondale CAAD10", "Black", 24, 28, "SmartFormed 6069 Alloy", 1350, 1845));
		catalog2.add(new RoadBike(20, "Giant Anyroad 1", "Black", 24, 28, "ALUXX-Grade Aluminum", 1249, 800));
		catalog2.add(new RoadBike(22, "Lapierre Aircode 300", "Black", 23, 28, "AIRCODE CARBON", 2949, 1850));
		...
		catalog4.add(new Hybrid(21, "Giant Escape 3", "Black/Blue", 20, 27, "ALUXX-Grade Aluminum",200, 375));
		catalog4.add(new Hybrid(24, "Carrera Subway 1", "Black", 22, 27, "Alloy", 300, 479.95));
	...
	private void createBicycles() {
		...
		// create Road bikes
		database.addBicycle(new RoadBike(18, "Boardman Road Comp", "Grey", 23, 29, "Alloy", 480, 649));
		database.addBicycle(new RoadBike(11, "Cannondale CAAD10", "Black", 26, 20, "SmartFormed 6069 Alloy", 1350, 1845));
		database.addBicycle(new RoadBike(20, "Giant Anyroad 1", "Black", 30, 21, "ALUXX-Grade Aluminum", 1249, 800));
		database.addBicycle(new RoadBike(22, "Lapierre Aircode 300", "Black", 28, 21, "AIRCODE CARBON", 2949, 1850));
		database.addBicycle(new RoadBike(21, "Raleigh", "White", 24, 20, "Steel", 2, 999.99));
		...
		database.addBicycle(new Hybrid(21, "Giant Escape 3", "Black/Blue", 20, 27, "ALUXX-Grade Aluminum", 200, 375));
		database.addBicycle(new Hybrid(24, "Carrera Subway 1", "Black", 22, 27, "Alloy", 300, 479.95));
		...
		database.addBicycle(new MotorisedBike("BH Evo City", "White", 28, 20, "Aluminium", 1800, 2650));
	}
	...
	private void createSalesTransaction() {
		...
		// create shopping baskets to add to transactions
				...
				Calendar sixtyDaysAgo = Calendar.getInstance();
				sixtyDaysAgo.add(Calendar.DAY_OF_YEAR, -60);
				database.addSalesTransaction(new SalesTransaction(10001, database.getEmployee(10002),
						database.getCustomer("Freddy Mercury"), basket1.getTotalCostValue(), "Account", sixtyDaysAgo.getTime(), basket1));

				// order from 45 days ago
				Calendar fortyFiveDaysAgo = Calendar.getInstance();
				fortyFiveDaysAgo.add(Calendar.DAY_OF_YEAR, -45);
				database.addSalesTransaction(new SalesTransaction(10002, database.getEmployee(10004),
						database.getCustomer("Tom Smith"), basket3.getTotalCostValue(), "Account", fortyFiveDaysAgo.getTime(), basket3));

				// order from 30 days ago
				Calendar thirtyDaysAgo = Calendar.getInstance();
				thirtyDaysAgo.add(Calendar.DAY_OF_YEAR, -30);
				database.addSalesTransaction(new SalesTransaction(10003, database.getEmployee(10004),
						database.getCustomer("Freddy Mercury"), basket4.getTotalCostValue(), "Account", thirtyDaysAgo.getTime(), basket4));

				// order from 25 days ago
				Calendar twentyFiveDaysAgo = Calendar.getInstance();
				twentyFiveDaysAgo.add(Calendar.DAY_OF_YEAR, -25);
				database.addSalesTransaction(new SalesTransaction(10004, database.getEmployee(10002),
						database.getCustomer("Freddy Mercury"), basket2.getTotalCostValue(), "Account", twentyFiveDaysAgo.getTime(), basket2));

				// order from 20 days ago
				Date twentyDaysAgo = new Date(System.currentTimeMillis() - 20 * 24 * 3600 * 1000);
				database.addSalesTransaction(new SalesTransaction(10005, database.getEmployee(10002),
						database.getCustomer("Tom Smith"), basket3.getTotalCostValue(), "Account", twentyDaysAgo, basket3));

				// order from 15 days ago
				Date fifteenDaysAgo = new Date(System.currentTimeMillis() - 15 * 24 * 3600 * 1000);
				database.addSalesTransaction(new SalesTransaction(10006, database.getEmployee(10002),
						database.getCustomer("Freddy Mercury"), basket4.getTotalCostValue(), "Account", fifteenDaysAgo, basket4));

				// order from 10 days ago
				Date tenDaysAgo = new Date(System.currentTimeMillis() - 10*24*3600*1000);
				database.addSalesTransaction(new SalesTransaction(10007, database.getEmployee(10004),
						database.getCustomer("Patrick Dunne"), basket1.getTotalCostValue(), "Account", tenDaysAgo,
						basket1));
				
				// order from 7 days ago
				Date sevenDaysAgo = new Date(System.currentTimeMillis() - 7*24*3600*1000);
				database.addSalesTransaction(new SalesTransaction(10008, database.getEmployee(10004),
						database.getCustomer("John Doe"), basket3.getTotalCostValue(), "Account", sevenDaysAgo,
						basket3));
				
				// order from 6 days ago
				Date sixDaysAgo = new Date(System.currentTimeMillis() - 6*24*3600*1000);
				database.addSalesTransaction(new SalesTransaction(10009, database.getEmployee(10002),
						database.getCustomer("Tom Smith"), basket2.getTotalCostValue(), "Account", sixDaysAgo, 
						basket2));
				
				// order from 4 days ago
				Date fourDaysAgo = new Date(System.currentTimeMillis() - 4*24*3600*1000);
				database.addSalesTransaction(new SalesTransaction(10010, database.getEmployee(10004),
						database.getCustomer("Patrick Dunne"), basket3.getTotalCostValue(), "Account", fourDaysAgo, 
						basket3));
				
				// order from 3 days ago
				Date threeDaysAgo = new Date(System.currentTimeMillis() - 3*24*3600*1000);
				database.addSalesTransaction(new SalesTransaction(10011, database.getEmployee(10002),
						database.getCustomer("Freddy Mercury"), basket2.getTotalCostValue(), "Account", threeDaysAgo, 
						basket2));
				
				// create orders for current date
				database.addSalesTransaction(new SalesTransaction(10012, database.getEmployee(10002),
						database.getCustomer("Tom Smith"), basket1.getTotalCostValue(), "Account", new Date(),
						basket1));
				database.addSalesTransaction(new SalesTransaction(10013, database.getEmployee(10004),
						database.getCustomer("Patrick Dunne"), basket2.getTotalCostValue(), "Account", new Date(), 
						basket2));
				database.addSalesTransaction(new SalesTransaction(10014, database.getEmployee(10004),
						database.getCustomer("John Doe"), basket3.getTotalCostValue(), "Account", new Date(), 
						basket3));
				database.addSalesTransaction(new SalesTransaction(10015, database.getEmployee(10002),
						database.getCustomer("Freddy Mercury"), basket2.getTotalCostValue(), "Account", new Date(),
						basket2));
